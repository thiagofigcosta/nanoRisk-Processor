...
bool isValidChar(char c){
    return c=='$'||(c>=48&&c<=57)||(c>=65&&c<=90)||(c>=97&&c<=122)||c=='-';
}
...
vector<string> translateFunc(string func,string args){
    ...
    if(!func.compare("slp")||!func.compare("sleep")){
        ...
        out.push_back(nRisk+target0);
        out.push_back(target2+bitset< 4 >(strToInt(arg1.arg)).to_string());
        return out;
    }//INTERPRETADAS
...
int main(int argc, char **argv){
    ...
    for(int i=1;i<machineCode.size();i+=2){
        if(machineCode[i].size()!=8||!isBinary(machineCode[i])){
            if(addresses.find(machineCode[i]) == addresses.end()){
                ...
                //cout<<"addresses:"<<machineCode[i]<<" = "<<addresses[machineCode[i]]<<endl;
    ...
    if(binary){
        if(separateFiles){
            ofstream dataFile(appendName(output,"_data"),ios::binary|ios::out);
            if(dataFile.is_open()){
            ...
            ofstream machineFile(appendName(output,"_inst"),ios::binary|ios::out);
        ...
        if(verilog){
            if(separateFiles){
                ...
                if(dataFile.is_open()){
                    if(coment)
                        dataFile<<"//data\n";
                    for(int i=0;i<dataSeg.size();i++)
                        dataFile<<verilog0<<"["<<i<<"]=8\'b"<<dataSeg[i]<<";"<<endl;
                    dataFile.close();
                }else{
                    error("Unable to create file");
                    return -1;
                ...
                if(machineFile.is_open()){
                    ...
                    for(int i=0;i<machineCode.size();i++)
                        machineFile<<verilog1<<"["<<(i)<<"]=8\'b"<<machineCode[i]<<";"<<endl;
                    machineFile.close();
                }else{
                    error("Unable to create file");
                    return -1;
                ...
                if(outFile.is_open()){
                    if(coment)
                        outFile<<"//data\n";
                    for(int i=0;i<dataSeg.size();i++)
                        outFile<<verilog0<<"["<<i<<"]=8\'b"<<dataSeg[i]<<";"<<endl;
                    if(coment)
                    ...
                    if(verilog0==verilog1)
                        offset=dataSeg.size();
                    for(int i=0;i<machineCode.size();i++)
                        outFile<<verilog1<<"["<<(i+offset)<<"]=8\'b"<<machineCode[i]<<";"<<endl;
                    outFile.close();
                }else{
                    error("Unable to create file");
                    return -1;
            ...
            if(separateFiles){
                ...
                for(int i=0;i<dataSeg.size();i++){
                    machineCode.insert(machineCode.begin()+i, dataSeg[i]);
                }
